...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `GazeboKotlin.swiftmodule/`
...
//      `import GazeboKotlin` above would fail at compile time.
//
//   2. The static archive `libGazeboKotlin.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "GazeboKotlin" }` and
//      `flattenPackage = "io.github.kotlinmania.gazebo"` configuration in
...
//      via the `GazeboKotlinLibrary` product.
//
...
final class GazeboKotlinExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "GazeboKotlin swift module imported cleanly")
    }
}
